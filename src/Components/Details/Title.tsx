...
const Title = () => {
    ...
    const handleAddfav = () => {
        ...
        if(showState.favourites.length === undefined){  // If favourites doesnt exist
            const fadData = [
                {
            ...
            return ;
        } else{                                      // Add favrourite to other favourites (by push)
    ...
    const changeStart = () => {                       // After click, change star
    ...
    const showStar =  () => {
        ...
        if(isInFav) {
            return (<Col  span={1}><h3 className="star activeStar">★</h3></Col>)  
        } else {
           return (<Col id="uncheckStar" span={1} onClick={e => {handleAddfav(); changeStart()}}><h3 className="star startUncheck ">☆</h3></Col> )
