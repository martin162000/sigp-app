...
const FoundFavourites = () => {
    ...
    return (
        <>
    {showState.favourites.length !== undefined ? showState.favourites.map((movie:any) => (
    <Content className="favourites leftFadeIn">
            <Row>
            <Col span={24}><h3>{movie.Title}</h3></Col>
            </Row>
            <Row className="favWrap">
            <div className="imgHolder"><img src={movie.Poster} alt="movie art"/></div>
            <Col xs={{ span: 24 }} lg={{ span: 12 }} className="favContent" >
                <div className="btnGroup">
                    <Button className="btns btnDetail" type="primary" onClick={e => handleClickDetails(movie.imdbID)}>Details</Button>
                    <Button className="btns" type="primary" danger onClick={e => handleClickDelete(movie.imdbID)}>Delete</Button>
                </div>
